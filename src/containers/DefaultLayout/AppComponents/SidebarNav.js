...
// import Logo from "assets/img/layouts/w-logo.svg";
import Logo from "assets/img/layouts/w-logo.svg";
import List from "assets/img/layouts/w-list.svg";
...
class AppSidebarNav extends Component {
  ...
  navMain = (key, classes) => {
    return (
      <NavItem key={key} className={classes.main}>
        <NavLink to="/" onClick={this.hideMobile}>
          <SVG src={Logo} />
  ...
  navMainBadge = () => {
    return <SVG className={classNames("nav-list")} src={List} />;
  };
